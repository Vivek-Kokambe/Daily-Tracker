...
export default function AnalyticsScreen({ waterGoal, calorieGoal, waterHistory, calorieHistory, habitCompletion, habits, sleepHistory }: AnalyticsScreenProps) {
  ...
  const demoCalories = useMemo(() => [1800, 2200, 1500, 1900, 2100, 1700, 1600, 2000, 1850, 1750, 1900, 2100, 1600, 1800, 2200, 1700, 1500, 2000, 1850, 1650, 2100, 1800, 1900, 1700, 2200, 1600, 2000, 1850, 1750, 1500], []);
  ...
  const habitData = useMemo(() => days.map((d) => { const completion = habitCompletion[d.key] || {}; const total = habits.length; const done = Object.values(completion).filter(Boolean).length; const percentage = total > 0 ? Math.round((done / total) * 100) : 75; return { day: d.day, percentage }; }), [days, habitCompletion, habits, demoHabits]);
  ...
  const waterTrend = waterData.length > 2 ? (() => { const first = waterData.slice(0, halfLen).reduce((s, d) => s + d.glasses, 0) / halfLen; const last = waterData.slice(halfLen).reduce((s, d) => s + d.glasses, 0) / (waterData.length - halfLen); return last > first ? 'up' : last < first ? 'down' : 'neutral'; })() : 'neutral';
  const calorieTrend = calorieData.length > 2 ? (() => { const first = calorieData.slice(0, halfLen).reduce((s, d) => s + d.calories, 0) / halfLen; const last = calorieData.slice(halfLen).reduce((s, d) => s + d.calories, 0) / (calorieData.length - halfLen); return last < first ? 'up' : last > first ? 'down' : 'neutral'; })() : 'neutral';
  const habitTrend = habitData.length > 2 ? (() => { const first = habitData.slice(0, halfLen).reduce((s, d) => s + d.percentage, 0) / halfLen; const last = habitData.slice(halfLen).reduce((s, d) => s + d.percentage, 0) / (habitData.length - halfLen); return last > first ? 'up' : last < first ? 'down' : 'neutral'; })() : 'neutral';
  const sleepTrend = sleepData.length > 2 ? (() => { const first = sleepData.slice(0, halfLen).reduce((s, d) => s + d.hours, 0) / halfLen; const last = sleepData.slice(halfLen).reduce((s, d) => s + d.hours, 0) / (sleepData.length - halfLen); return last > first ? 'up' : last < first ? 'down' : 'neutral'; })() : 'neutral';
  ...
  return (
    <motion.div variants={container} initial="hidden" animate="show" className="px-5 pt-2 pb-20 antialiased">
      ...
      <motion.div variants={item} className="flex gap-2 mb-4">
        ...
        <motion.button whileTap={{ scale: 0.95 }} onClick={() => setComparisonMode(isComparing ? 'this-week' : 'vs-last-week')} className="flex-shrink-0 px-3 py-1.5 rounded-xl text-[10px] font-bold cursor-pointer transition-all" style={{ backgroundColor: isComparing ? 'rgba(59,130,246,0.15)' : 'rgba(255,255,255,0.03)', border: isComparing ? '1.5px solid rgba(59,130,246,0.3)' : '1px solid rgba(255,255,255,0.06)', color: isComparing ? '#3b82f6' : '#8a8a9f' }}>
      ...
      {/* Water Chart */}
      <motion.div variants={item} className="rounded-2xl p-4 mb-4" style={{ backgroundColor: '#1a1a2e', ...CARD_STYLE }}>
        <ChartSummaryCard label="Avg Water" value={avgWater} unit="glasses/day" color="#00b4d8" trend={waterTrend as 'up' | 'down' | 'neutral'} trendLabel={waterTrend === 'up' ? '+12%' : waterTrend === 'down' ? '-8%' : '--'} />
        ...
        <div className="h-40">
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={waterData}>
              ...
              <Area type="monotone" dataKey="glasses" stroke="#00b4d8" strokeWidth={2.5} fill="url(#waterAreaFill)" dot={{ r: 4, fill: '#00b4d8', strokeWidth: 0 }} activeDot={{ r: 6, fill: '#00b4d8', strokeWidth: 2, stroke: '#0a0a0f' }} />
            </AreaChart>
          </ResponsiveContainer>
        </div>
      </motion.div>
      ...
      {/* Calorie Chart */}
      <motion.div variants={item} className="rounded-2xl p-4 mb-4" style={{ backgroundColor: '#1a1a2e', ...CARD_STYLE }}>
        <ChartSummaryCard label="Avg Calories" value={avgCalories} unit="kcal/day" color="#ff6b35" trend={calorieTrend as 'up' | 'down' | 'neutral' | undefined} trendLabel={calorieTrend === 'up' ? 'On target' : calorieTrend === 'down' ? 'Over budget' : undefined} />
        <div className="flex items-center gap-2 mb-3">
      ...
      {/* Habit Chart */}
      <motion.div variants={item} className="rounded-2xl p-4 mb-4" style={{ backgroundColor: '#1a1a2e', ...CARD_STYLE }}>
        <ChartSummaryCard label="Avg Completion" value={avgHabitPercent} unit="%" color="#4caf50" trend={habitTrend as 'up' | 'down' | 'neutral' | undefined} trendLabel={habitTrend === 'up' ? 'Improving' : habitTrend === 'down' ? 'Declining' : undefined} />
        <div className="flex items-center gap-2 mb-3">
      ...
      {/* Sleep Chart */}
      <motion.div variants={item} className="rounded-2xl p-4 mb-4" style={{ backgroundColor: '#1a1a2e', ...CARD_STYLE }}>
        <ChartSummaryCard label="Avg Sleep" value={avgSleepHours} unit="hrs/night" color="#9c27b0" trend={sleepTrend as 'up' | 'down' | 'neutral' | undefined} trendLabel={sleepTrend === 'up' ? 'Improving' : sleepTrend === 'down' ? 'Declining' : undefined} />
        <div className="flex items-center gap-2 mb-3">
      ...
      <motion.div variants={item} className="rounded-2xl p-4 mb-4 relative overflow-hidden" style={{ background: 'linear-gradient(145deg, rgba(59,130,246,0.08), rgba(59,130,246,0.03))', border: '1px solid rgba(59,130,246,0.15)' }}>
        <div className="flex items-center gap-2 mb-3">
        ...
        <div className="flex flex-col gap-2">
          {insights.slice(0, visibleInsights).map((insight, idx) => (
            <motion.p key={idx} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.1 }} className="text-[11px] font-normal leading-relaxed animate-cursor-blink" style={{ color: '#c8c8d8' }}>
              {insight}
            </motion.p>
          ))}
        </div>
      ...
      {/* Success Message */}
      <motion.div variants={item} className="rounded-2xl p-4 flex items-center gap-3" style={{ background: 'linear-gradient(135deg, rgba(0,180,216,0.1), rgba(0,180,216,0.05))', border: '1px solid rgba(0,180,216,0.15)' }}>
